...
There are some issues with the gradient descent model. You can imagine, that if we are traversing down the gradient, we may converge onto a local minima rather than the global minima, thus leading to a less-than-optimal model. There are many methods for setting the initial weights of the model to help reduce the likelihood of this. I use the He method of initialisaiton, where each of the weights of each layer are randomly sampled from a normal distribution with $\mu = 0$ and $\sigma = \sqrt{\frac{2}{n}}$ where $n$ is the number of inputs into the layer. This is reguarded good practice for ReLU layers. 
